import { DemoGenericService } from "./demo-generic.service.intf";
...
@Injectable()
export class DemoGenericServiceImpl implements DemoGenericService {
	public constructor(private store: Store<DemoGenericSearchState>) {}

	public getSearchState(): Observable<StarkSearchState<GenericObjectSearchCriteria>> {
	...
	public search(criteria: GenericObjectSearchCriteria): Observable<GenericObject[]> {
		...
		return of(MOVIES).pipe(
			map((genericObjects: GenericObject[]) => {
				return genericObjects.filter((genericObject: GenericObject) =>
